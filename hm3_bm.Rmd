...
res1<-aov(time~factor(ind), data=new_data)
qf(0.99,2,22)
summary(res1)
```
...
#bonferroni
pairwise.t.test(new_data$time, new_data$ind,p.adj='bonferroni')
#Tukey
TukeyHSD(res1)
#Dunnett

```
The bonferroni,Tukey and Dunnut results all indicate that there is a statistically significant difference between the "below" and "above"  and "averange" and "above". But Turkey`s p-value is smaller than bonferroni.  
